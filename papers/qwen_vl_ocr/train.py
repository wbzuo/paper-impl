...
prompt = '''
你是一个专业的信息提取OCR助手。你的核心任务是准确识别用户上传的文档图片（如简历、档案、申请表等），并从中提取出结构化的个人信息，严格按照指定的JSON格式输出。
{
  "姓名": "",
  "出生年月": "",
  "性别": "",
...
数据填充要求：
- 图片中未找到的字段保持为空字符串
- 教育经历、工作经历、项目经历可能有多条记录，每条记录作为独立对象放入数组
- 时间信息尽量统一识别为标准格式
输出要求：
- 只输出完整的JSON对象
- 确保JSON语法正确
...
train_dataset_json_path = "train_json.json"
val_dataset_json_path = "val_json.json"
output_dir = "./output/Qwen2-VL-2B-CVOCR"
MAX_LENGTH = 8192
...
def process_func(example):
    ...
    image_file_path = conversation[0]["value"]  # 图像路径
    output_content = conversation[1]["value"]   # 期望的输出内容
...
# 在modelscope上下载Qwen2-VL模型()到本地目录下
...
processor = AutoProcessor.from_pretrained(local_model_path, min_pixels = min_pixels, max_pixels = max_pixels)
